...
if (!isset($_SESSION)) {
 ...
 $data= sprintf("SELECT COUNT(*) from xuanzhuanmuma");
 ...
 $selectsql =  sprintf("SELECT usename from xuanzhuanmuma WHERE usename=%s",GetSQLValueString($_SESSION[MM_Username], "text"));
  mysql_select_db($database_shujuku, $shujuku);
 ...
 if($_SESSION[MM_Username] != NULL)
 {
	 if( $maipiaoflag[0] !=0)
	 {
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form")) {
			if($searchflag)
			{
			 $deleteSQL = sprintf("DELETE FROM xuanzhuanmuma WHERE usename=%s",
                       GetSQLValueString($_SESSION[MM_Username], "text"));

		  mysql_select_db($database_shujuku, $shujuku);
		  $Result1 = mysql_query($deleteSQL, $shujuku) or die(mysql_error());
		  $insertSQL = sprintf("INSERT INTO xuanzhuanmuma (usename, shunxu) VALUES (%s, %s)",
							   GetSQLValueString($_SESSION[MM_Username], "text"),
							   GetSQLValueString($_POST['shunxu'], "int"));
		
		  mysql_select_db($database_shujuku, $shujuku);
		  $Result1 = mysql_query($insertSQL, $shujuku) or die(mysql_error());
		  echo "<script>alert('更改预约成功'); location.href='xuanzhuanmuma.php'</script>";
			}
			else
			{
				 $insertSQL = sprintf("INSERT INTO xuanzhuanmuma (usename, shunxu) VALUES (%s, %s)",
		  ...
		  mysql_select_db($database_shujuku, $shujuku);
		  $Result1 = mysql_query($insertSQL, $shujuku) or die(mysql_error());
		  echo "<script>alert('预约成功'); location.href='xuanzhuanmuma.php'</script>";
			}
		  }
		 }else
		 {
...
<div class="container">
	<div class="row clearfix">
		<div class="col-md-12 column">
             <h1 class="text-center">旋转木马项目</h1>
             <br/>
             <br/>
			<div class="row clearfix ">
				<div class="col-md-6 column">
               <img alt="300*200" src="picture/旋转木马.jpeg" class="img-thumbnail" />
				</div>
				<div class="col-md-6 column">
